...
const Profile = () => {
    ...
    const { open, close, state } = useTonConnectModal(); // ✅ Контроль над модалкой

    console.log("🔥 Текущий статус кошелька:", walletAddress ? `Подключен: ${walletAddress}` : "Не подключен");
    ...
    const handleWalletClick = () => {
        ...
        if (walletAddress) {
            ...
            open(); // ✅ Открываем модалку с 'Copy Address' и 'Disconnect'
        } else {
            console.log("🔗 Кошелек не подключен. Открываю модалку подключения...");
            open(); // ✅ Открываем модалку подключения
        }
    };
